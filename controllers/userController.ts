...
import {
  ...
  changeAvatarService,
  changeBankingInfoService,
  changeGenderServices,
  changeOnlineStatusService,
  changePasswordService,
  changePhoneNumberServices,
  favoriteProductService,
  followStoreService,
  getAllUserServices,
  getFavoriteListOfAUserService,
  getFollowStoreService,
  getUserInfoServices,
  removeFromCartServices,
  requestOpenStoreService,
...
export const changePassword = async (req, res, next) => {
  const { userId, email }: { userId: string; email: string } = req.user;
  const {
    currentPassword,
    newPassword,
...
export const changeAddress = async (req, res, next) => {
  ...
  const {
    ...
    ward,
    city,
  }: { street: string; district: string; ward: string; city: string } =
    req.body;

  try {
    const newAddress = await changeAddressService(
      userId,
      email,
      street,
      district,
...
export const changeBankingInfo = async (req, res, next) => {
  ...
  const {
    ...
    accountOwner,
    accountNumber,
  }: { bankName: string; accountOwner: string; accountNumber: string } =
    req.body;

  try {
    const newBankingInfo = await changeBankingInfoService(
      userId,
      email,
      bankName,
      accountOwner,
...
export const followStore = async (req, res, next) => {
  ...
  const { followedStoreId }: { followedStoreId: string } = req.params;
...
export const unfollowStore = async (req, res, next) => {
  const { userId }: { userId: string } = req.user;
  const { unfollowedStoreId }: { unfollowedStoreId: string } = req.params;
...
export const favoriteProduct = async (req, res, next) => {
  const { userId }: { userId: string } = req.user;
  const { favoriteProductId }: { favoriteProductId: string } = req.params;
...
export const unfavoriteProduct = async (req, res, next) => {
  const { userId }: { userId: string } = req.user;
  const { unfavoriteProductId }: { unfavoriteProductId: string } = req.params;
...
export const changeGender = async (req, res, next) => {
  const { userId }: { userId: string } = req.user;
  const { gender }: { gender: string } = req.body;
...
export const changeAvatar = async (req, res, next) => {
  const { userId }: { userId: string } = req.user;
  const { avatarLink }: { avatarLink: string } = req.body;
...
export const requestOpenStore = async (req, res, next) => {
  const { userId }: { userId: string } = req.user;
  const requestData: IStoreRequest = req.body;
...
export const buy = async (req, res, next) => {
  const { userId }: { userId: string } = req.user;
  const { isCodQuery }: { isCodQuery: string } = req.query;

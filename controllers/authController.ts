...
export const signUp = async (req, res, next) => {
  ...
  try {
    const { token, newUser } = await signUpService(
      email,
      password,
      fullName,
      devicetokenfcm
...
export const googleAuth = async (req, res, next) => {
  ...
  const { devicetokenfcm }: { devicetokenfcm: string } = req.headers;
  try {
    const { token, user } = await googleLoginService(
      email,
      fullName,
      devicetokenfcm
    );
...
export const resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    if (result == true)
      res.status(200).json({
        message:
          'Reseted password successfully. Email with new password has been sent to your email',
      });
  } catch (err) {
    next(err);
  }

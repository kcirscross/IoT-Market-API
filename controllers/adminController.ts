import {
  approveStoreRequestServices,
  banUserServices,
  declineStoreRequestServices,
  getAllRequestServices,
  getRequestDetailServices,
  unbanUserServices,
} from '../services/adminServices.js';

export const declineStoreRequest = async (req, res, next) => {
  const {
    params: { storeRequestId },
    body: { reasonForReject },
  } = req;
  try {
    const storeRequest = await declineStoreRequestServices(
      storeRequestId,
      reasonForReject
    );

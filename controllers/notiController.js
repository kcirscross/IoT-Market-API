import { sendNotificationToOneDeviceService, setReadService, viewAllUserNotificationService, } from '../services/notiServices.js';
export const sendNotificationToOneDevice = async (req, res, next) => {
    ...
    try {
        const result = await sendNotificationToOneDeviceService(messageData, userId);
        if (result)
            res
                .status(200)

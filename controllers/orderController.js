import { createOrderServices, getAllOrderServices, getBuyerOrdersServices, getOrderDetailServices, getOrderDetailServicesAdmin, getSellerOrdersServices, getStoreOrdersForAdminServices, setReceiveOrderService, } from '../services/orderServices.js';
export const createOrder = async (req, res, next) => {
    const paymentData = req.query;
    try {
        const order = await createOrderServices(paymentData);
        if (order)
            res.status(200).json({ message: 'Buy successfully', order });
    }
    catch (err) {
        next(err);
    }
...
export const getSellerOrders = async (req, res, next) => {
    ...
    try {
        ...
        if (orders)
            res.status(200).json({ message: 'Buy successfully', orders });
    }
    catch (err) {
        next(err);
    }

...
export const calculateShippingFeeService = async (
  userId: string,
  productToBuy
) => {
  //Get User
  ...
  const productList = await Promise.all(
    productToBuy.map(async (product) => {
      const productDetail: IProduct = await getProductInfoService(
        product.productId
      );
  ...
  const totalProductCost = productList.reduce(
    (sum, product) =>
      (sum += parseInt(product.productId.price) * product.quantity),
    0
  );
  ...
  const shippingFeeList = await Promise.all(
    Object.keys(productListInCartGroupedByStore).map(async (store) => {
      let ownerOfProduct_Address: IStore | IUser = await Store.findById({
        _id: store,
      }).select('address_ID ghnShopId');

      if (!ownerOfProduct_Address)
        ownerOfProduct_Address = await User.findById({ _id: store }).select(
          'address_ID ghnShopId'
        );

      const totalSize = productListInCartGroupedByStore[store].reduce(
        (totalSize, product) => {
          ...
          if (totalSize.width < product.width) totalSize.width = product.width;
          return {
  ...
  const totalShippingFee = shippingFeeList.reduce(
    (sum, fee) => (sum += fee.shippingFee),
    0
  );
...
export const trackingShippingService = async (deliveryCode: string) => {

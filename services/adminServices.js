...
// import User from "../models/User.js";
export const declineStoreRequestServices = async (storeRequestId, reasonForReject) => {
    if (!reasonForReject)
        throw createError(400, 'Please provide for user your reason for declining this request');
    const storeRequest = await StoreRequest.findOne({
        _id: storeRequestId,
        status: 'Pending',
    });
    if (!storeRequest)
        throw createError(404, `No store request with id ${storeRequestId} or the request has already been approved/decline`);
    ...
    await sendNotificationToOneDeviceService(messageData, storeRequest.requesterId.toString());
    return storeRequest;
};
export const approveStoreRequestServices = async (storeRequestId) => {
    const storeRequest = await StoreRequest.findOne({
        _id: storeRequestId,
        status: 'Pending',
    });
    ...
    await storeRequest.save();
    const { requesterId: ownerId, displayName, description, shopImage, address, phoneNumber, } = storeRequest;
...
export const getRequestDetailServices = async (requestId) => {
    const request = await StoreRequest.findById(requestId).populate('requesterId');
    if (!request)
        throw createError(404, 'There is no request with this id in database');
    return request;
};
export const banUserServices = async (userId) => {
    const userToBan = await User.findOne({
        _id: userId,
        accountStatus: 'Active',
    });
    if (!userToBan)
        throw createError(404, `No user with id ${userId} or the user has already been banned`);
    userToBan.accountStatus = 'BAN';
    await userToBan.save();
    return userToBan;
};
export const unbanUserServices = async (userId) => {

...
import Order, { IOrder, IPaymentInfo } from '../models/Order.js';
import Shipping, { IShipping } from '../models/Shipping.js';
...
export const createOrderServices = async (paymentData: IPaymentInfo) => {
  ...
  const secureHash: string = paymentData.vnp_SecureHash;
  ...
  const signData: string = qs.stringify(paymentData, { encode: false });
  const hmac: crypto.Hmac = crypto.createHmac('sha512', secretKey);
  const signed: string = hmac.update(signData, 'utf-8').digest('hex');

  if (secureHash === signed) {
    const userId: string = paymentData.vnp_OrderInfo.split('_')[0];
    const shippingId: string = paymentData.vnp_OrderInfo.split('_')[1];
    await sendNotificationToOneDeviceService(messageDataForUser, userId);

    const shipping: IShipping = await Shipping.findById({ _id: shippingId });
    const user: IUser = await User.findById({ _id: userId });
    ...
    const orders = Promise.all(
      shipping.shippingFeeList.map(async (shippingFee) => {
        ...
        if (shippingFee.isStore) {
          ownerRole = 'Store';
          const owner: IUser = await User.findOne({
        ...
        const order = await Order.create({
          ...
          expected_delivery_time:
            createdOrderToVPN.data.data.expected_delivery_time,
...
export const createOrderCODServices = async (
  shipping: IShipping,
  userId: string
) => {
  const user: IUser = await User.findById({ _id: userId });
  ...
  const orders = Promise.all(
    shipping.shippingFeeList.map(async (shippingFee) => {
      ...
      const order = await Order.create({
        ...
        VNPay: {
          vnp_Amount:
            (shipping.totalShippingFee + shipping.totalProductCost) * 100,
        },
      });
...
export const getBuyerOrdersServices = async (userId: string) => {
  const orders: IOrder[] = await Order.find({ userId }).sort({
    createdAt: -1,
  });
  return orders;
};

export const getSellerOrdersServices = async (
  userId: string,
  storeId: string
) => {
...
export const getOrderDetailServices = async (orderId: string) => {
  const order: IOrder = await Order.findById(orderId);

  if (!order) throw createError(400, 'No order found');

  const isReceived = order.shippingLogs.filter((shippingLog) => {
    return shippingLog.status === 'delivered';
  });

  if (isReceived && isReceived.length > 0) return order;
...
export const getOrderDetailServicesAdmin = async (orderId: string) => {
  const order: IOrder = await Order.findById(orderId)
    .populate({
      path: 'productsList',
      populate: {
...
export const setReceiveOrderService = async (orderId: string) => {
...
export const getBuyerOrdersForAdminServices = async (userId: string) => {
  const orders: IOrder[] = await Order.find({ userId }).sort({
    createdAt: -1,
  });
  return orders;
...
export const getStoreOrdersForAdminServices = async (storeId: string) => {
  const orders: IOrder[] = await Order.find({
    ownerId: storeId,
  })
    .populate('userId')
